...
3. Give your app a name (e.g., "My AI Assistant") and select your workspace
4. Click **Create App**
...
You'll configure the Event Subscriptions URL after deploying your agent, but for now, navigate to **Event Subscriptions** in your Slack App settings and prepare to enable it. You'll need to subscribe to these bot events:

- `app_mention` — When the bot is @mentioned
- `message.im` — Direct messages to the bot

Don't enable it yet — we'll come back to this after deployment.
...
## Step 2 — Create your Slack Agent project
...
## Step 4 — Create your Slack Agent

First, create the base `SlackAgent` class at `src/slack.ts`. This class handles OAuth, request verification, and event routing. You can view the [full implementation on GitHub](https://github.com/cloudflare/awesome-agents/blob/69963298b359ddd66331e8b3b378bb9ae666629f/agents/slack/src/slack.ts).

Now create your agent implementation at `src/index.ts`:
...
Now that your agent is running locally, you need to expose it to Slack. Use [Cloudflare Tunnel](/cloudflare-one/connections/connect-networks/) to create a secure tunnel:
...
:::note
Cloudflare Tunnel URLs are temporary. When testing locally, you'll need to update the Request URL each time you restart the tunnel.
:::

### Install your app to Slack

Visit `http://localhost:8787/install` in your browser. This will redirect you to Slack's authorization page. Click **Allow** to install the app to your workspace.
...
After deploying, you'll get a production URL like:
...
- **Single workspace**: Just install it via `https://my-slack-agent.your-account.workers.dev/install`
...
2. User clicks "Allow" → Slack redirects to `/accept` with an authorization code
