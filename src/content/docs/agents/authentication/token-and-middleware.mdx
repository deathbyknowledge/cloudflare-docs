...
- **Auth hooks** via [`routeAgentRequest`](/agents/api-reference/calling-agents/) — built-in hooks that run before each request is forwarded. Best for straightforward token or JWT checks with no external framework.
- **Custom routing** via [`getAgentByName`](/agents/api-reference/calling-agents/) — look up Agent instances manually after running your own middleware. Best when you use a framework like [Hono](https://hono.dev) and want to use its middleware ecosystem.
